...
> **Namenskollision, kein Widerspruch:** `dotnet-modulith-bridge`s `SharedKernel`-Projekt heißt
> genauso wie das DDD-Muster oben, meint aber das Gegenteil — reines technisches Plumbing
> (`IDomainEvent`, `IClock`), Domänensprache dort ist ausdrücklich verboten. Das DDD-Muster
> „Shared Kernel" (gemeinsamer *Domänen*-Code) hat in dieser Suite keine Entsprechung.
>
> **Separate Ways** ist der Fachbegriff für `modulith-thinking`s Konzern/Tochterunternehmen-Bild:
...
Für den konkreten Ordner-/Projekt-Zuschnitt dieser Suite (Feature-Ordner, `.csproj`-Zuschnitt,
Zimmertiefe, Contracts-Grenze): `angular-modulith-bridge` bzw. `dotnet-modulith-bridge`. Diese
Tabellen bleiben die generische DDD-Übersetzung, die Brücken machen sie Bazaar-konkret.

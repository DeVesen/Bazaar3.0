...
describe('StammdatenPopup', () => {
  ...
  it('dialogTitle re-evaluates when the active language changes after render', () => {
    ...
    translate.setTranslation('en', { stammdatenPopup: { createTitle: 'New {{entity}}' }, common: { cancel: 'Cancel', save: 'Save' } });
    translate.use('en');
    fixture.detectChanges();
  ...
  it('shows the translated success toast after a successful save', () => {
    ...
    expect(addSpy).toHaveBeenCalledWith(expect.objectContaining({ severity: 'success', summary: translate.instant('stammdatenPopup.saved', { entity: 'Marke' }) }));
  });

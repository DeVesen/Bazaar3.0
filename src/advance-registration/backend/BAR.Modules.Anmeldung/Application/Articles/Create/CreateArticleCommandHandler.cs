...
namespace BAR.Modules.Anmeldung.Application.Articles.Create;

public sealed class CreateArticleCommandHandler(
    IArticleRepository articles, INumberBlockRepository blocks, IBetriebModuleApi betrieb, IClock clock)
{
    public async Task<CreateArticleResultDto> HandleAsync(CreateArticleCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            throw new Anmeldung.Contracts.ArticleNumberConflictException(

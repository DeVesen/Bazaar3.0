...
public class BrandsEndpointsTests : IClassFixture<PostgresWebApplicationFactory>
{
    ...
    public async Task Put_AsAdmin_Returns200WithUpdatedName()
    {
        ...
        var created = await seller.PostAsJsonAsync("/api/brands", new { name = $"Z-{Guid.NewGuid():N}" }, TestContext.Current.CancellationToken);
        var body = await created.Content.ReadFromJsonAsync<JsonElement>(TestContext.Current.CancellationToken);
        var id = body.GetProperty("id").GetString();
        var admin = await AuthenticateAsAdminAsync();

        var newName = $"Renamed-{Guid.NewGuid():N}";
        var response = await admin.PutAsJsonAsync($"/api/brands/{id}", new { name = newName, original = true }, TestContext.Current.CancellationToken);

...
namespace BAR.Application.UnitTests.Verkaeuferverwaltung.Sellers.Invite;
...
public class InviteSellerCommandHandlerTests
{
    ...
    public async Task HandleAsync_ExistingSeller_GeneratesTokenAndPersists()
    {
        ...
        var result = await handler.HandleAsync(seller.Id, TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_UnknownId_ThrowsNotFound()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.HandleAsync("missing", TestContext.Current.CancellationToken));
    }
}

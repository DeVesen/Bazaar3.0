...
namespace BAR.Application.UnitTests.Verkaeuferverwaltung.Sellers.List;

/// <summary>
/// GetSellersQueryHandler joint nicht mehr per SQL (ISellerListQuery ist weg),
/// sondern komponiert in-memory ueber IStammdatenModuleApi/IAnmeldungModuleApi -
/// siehe Handler-Kommentar. Diese Tests mocken darum die drei Bausteine statt
/// eines einzigen Query-Ports.
/// </summary>
public class GetSellersQueryHandlerTests
{
    ...
    private GetSellersQueryHandler CreateHandler() => new(_sellers.Object, _stammdaten.Object, _anmeldung.Object);
    ...
    private void SetUpDefaults(IReadOnlyList<Seller> all, IReadOnlyDictionary<string, SellerBlockSummaryDto>? summaries = null)
    {
        ...
        {
            _stammdaten.Setup(t => t.GetSellerTypeConditionsAsync(typeId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new SellerTypeConditionsDto(typeId, "Standard", 15m, 0.5m));
        }

        _anmeldung.Setup(a => a.GetBlockSummariesForSellersAsync(It.IsAny<IReadOnlyCollection<string>>(), It.IsAny<CancellationToken>()))
    ...
    public async Task HandleAsync_SellerTypeConditionsUnresolvable_ExcludesThatSellerFromResult()
    {
        ...
        _stammdaten.Setup(t => t.GetSellerTypeConditionsAsync("unknown", It.IsAny<CancellationToken>()))

...
namespace BAR.Application.UnitTests.Betrieb.Public.GetInfo;
...
public class GetPublicInfoQueryHandlerTests
{
    ...
    private GetPublicInfoQueryHandler CreateHandler() => new(_settings.Object, _stammdaten.Object);
    ...
    public async Task HandleAsync_SettingsRowWithNullDefaultTypeId_ReturnsNullConditionsWithoutLookup()
    {
        ...
        Assert.Null(result.DefaultConditions);
        _stammdaten.Verify(t => t.GetSellerTypeConditionsAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}

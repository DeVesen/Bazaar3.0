...
namespace BAR.Modules.Stammdaten.Application.MasterData.Categories.Delete;

public sealed class DeleteCategoryCommandHandler(ICategoryRepository categories, IAnmeldungModuleApi anmeldung)
{
    public async Task HandleAsync(string id, CancellationToken cancellationToken)
    {
        ...
        var count = await anmeldung.CountArticlesWithCategoryNameAsync(category.Name, cancellationToken);
        if (count > 0)
        {

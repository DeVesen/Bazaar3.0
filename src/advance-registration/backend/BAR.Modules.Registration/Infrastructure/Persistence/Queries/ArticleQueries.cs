...
namespace BAR.Modules.Registration.Infrastructure.Persistence.Queries;

public sealed class ArticleQueries(RegistrationDbContext dbContext) : IArticleQueries
{
    ...
    /// <summary>
    /// A deviation from the spec: <c>EF.Functions.ILike(a.Number.ToString(), term)</c>
    /// throws an <see cref="InvalidOperationException"/> at runtime
    /// ("could not be translated") - the EF Core/Npgsql provider does not
    /// translate <c>int.ToString()</c> inside a LINQ predicate to SQL
    /// (verified empirically against the real Postgres test container).
    /// Therefore the number substring match is handled separately here: the
    /// candidates (already narrowed down by brand/category/sellerId) are
    /// loaded with only Id+Number, and the substring comparison runs locally
    /// in memory - uncritical for the bazaar's data volume. The matching ids
    /// then flow back into a server-side <c>WHERE id IN (...) OR ILike(...)</c>.
    /// </summary>

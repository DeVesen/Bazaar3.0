...
- Lokaler Dev-Start: `launchSettings.json` Profil `http` → `http://localhost:5001`. Postgres lokal auf Port `6543`, Connection String in `appsettings.Development.json` (`Host=localhost;Port=6543;Database=bar;Username=bar;Password=bar`) — passend zu `compose.db-only.yaml`, das ebenfalls auf `6543:5432` ausweicht (Kollisionsvermeidung mit einer lokal laufenden Postgres auf `5432`, siehe VPROJ-S03).

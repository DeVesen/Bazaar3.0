...
**Spec:** [`docs/superpowers/specs/2026-09-09-r01-zugang-design.md`](../specs/2026-09-09-r01-zugang-design.md) — executors read both. Die Spec dokumentiert drei Klärungen mit dem Requester (Abschnitt „Klärung vorab"): Nummernblock-Zuweisung bei Registrierung wird umgesetzt, ihr Umfang ist auf Allocator + `GET /api/blocks/mine` begrenzt, und `POST /api/auth/register` nimmt volle Stammdaten entgegen (nicht nur `email`/`password` wie `api/auth.md` noch zeigt — diese Doku-Datei ist an der Stelle veraltet und bekommt einen separaten Korrektur-Task, siehe unten).
...
- Passwort-Hash-Implementierung: `BCrypt.Net-Next` (NuGet), Work-Factor 12 — Spec/Entity-Doku lassen „bcrypt oder Argon2" als Implementierungsdetail offen, diese Wahl ist final für R01.
- JWT-Claim-Mapping: `JwtBearerOptions.MapInboundClaims = false`, Issuer schreibt literal `sub`/`role`/`exp` (keine ASP.NET-Standard-URI-Claims) — Kompatibilitätsvoraussetzung für `jwt-decoder.ts` im Frontend.
- `BAR.Application`-Ordnerstruktur: Feature-Ordner pro Use Case (`Auth/Register/`, `Auth/Login/`, …). `BAR.Domain`-Ordnerstruktur: Aggregat-Ordner (`Sellers/`, `SellerTypes/`, `Settings/`, `Auth/`, `NumberBlocks/`) statt Feature-Ordner (`api/cross-cutting.md` erlaubt Feature-Ordner nur in Application/Api).
- JWT-Signing-Key (Dev): fester 256-Bit-Dev-Key in `appsettings.Development.json`, Produktions-Key ausschließlich über `Jwt__SigningKey`-Env-Var.
...
**Carries:** Epic_Login AC-5/AC-6/AC-7/AC-8/AC-9/AC-10/AC-11, `api/auth.md` Abschnitt 2 (Token-Ausstellung/Blockvergabe), Spec „Klärung vorab" Punkt 3 (Stammdaten-Pflichtfelder im Request-Body — Requester-Entscheidung 2026-09-09).

**Done when:** Gültige Registrierung legt Seller (mit vollen Stammdaten) + `defaultBlockCount` Blöcke an und liefert ein Token-Paar; doppelte E-Mail wirft `ConflictException("seller.email_taken", ...)`; fehlender `defaultTypeId` wirft `ConflictException("registration.not_enabled", ...)`; fehlendes Stammdaten-Pflichtfeld wirft Validierungsfehler (400 über Task 13).
...
**Carries:** `api/auth.md` Endpoints 1–3 (Token-Hülle, Fehlerfälle — Request-Body von `register` erweitert um Stammdaten, siehe Spec „Klärung vorab" Punkt 3); Epic_Login AC-1/AC-2/AC-8/AC-9/AC-10/AC-11.
...
**Carries:** Epic_Login Abschnitt 6, AC-5/AC-6/AC-7/AC-8. Pflichtfeld-Validierung markiert fehlende Felder ohne Submit (AC-5, jetzt inklusive der Stammdaten-Pflichtfelder aus `entities/verkaeufer.md` — Requester-Entscheidung 2026-09-09, siehe Spec „Klärung vorab" Punkt 3).

**Done when:** Submit-Button ist deaktiviert solange Passwort-Stärke unter „Mittel" (AC-6), Nicht-Übereinstimmung von Passwort/Bestätigung zeigt Fehler ohne Submit (AC-7), `@Input() emailTakenError` zeigt den AC-8-Text mit Login-Link, fehlendes Stammdaten-Pflichtfeld verhindert Submit.
...
> **Reihenfolge-Hinweis:** `Countdown` (Task 28) entsteht nach diesem Task. Beim
> Ausführen in Task-Reihenfolge zuerst Task 28 vorziehen (Domain-Logik+Component
> ohne Abhängigkeit zu login-info-panel), dann Task 26 — oder hier zunächst ohne
> `<app-countdown>` einen `data-testid="countdown-box"`-Platzhalter-`div`
> committen und ihn in Task 28/29 durch die echte Komponente ersetzen. Diese
> Plan-Version geht von vorgezogenem Task 28 aus (Reihenfolge beim Ausführen
> entsprechend tauschen, Task-Nummern bleiben zur Nachverfolgbarkeit stabil).
...
- Create: `src/advance-registration/frontend/BAR.App/src/app/features/login/components/login-layout.html`
...
**Interfaces:**
- Consumes: `AuthApiService` (Task 21), `AuthService.login()` (bestehend, R00), `PublicInfoService` (Task 26), `LoginLayout`/`LoginInfoPanel`/`LoginForm` (Tasks 24/26/27)
...
**Interfaces:**
- Consumes: `AuthApiService` (Task 21), `AuthService.login()` (bestehend), `RegistrierungForm`/`RegistrierungFormValue` (Task 25)
...
## Self-Review (Autor, vor Übergabe)

**Spec-Abdeckung:** Alle Abschnitte der Spec sind abgedeckt — Backend-Architektur (Tasks 1–20), Frontend-Komponenten (Tasks 21–31), beide Klärungspunkte der Spec (Blockvergabe bei Registrierung: Tasks 6/14; Stammdaten im Register-Body: Tasks 14/19/25/30). Einzige bewusste Lücke: `POST /api/auth/set-password` bleibt Platzhalter (R01-Roadmap verschiebt ihn nach R06) — kein Task dafür, wie in der Spec dokumentiert.

**Platzhalter-Scan:** Keine TBD/TODO, keine "Similar to Task N"-Verweise ohne Code — jeder Code-Schritt trägt vollständigen, copy-paste-fähigen Code. Einzige Ausnahme bewusst markiert: der BCrypt-Hash-Platzhalter in Task 9 (`REPLACE_WITH_GENERATED_BCRYPT_HASH`) ist eine Ausführungsanweisung, kein offener Design-Punkt — der Task nennt den exakten Befehl zum Ersetzen.

**Typkonsistenz geprüft:** `RegisterCommand`/`RegisterRequest`/`RegistrierungFormValue`/`RegisterPayload` tragen an allen vier Stellen dieselben acht Felder in gleicher Bedeutung (email, password, firstName, lastName, address, postalCode, city, phone). `TokenPairResult`/`TokenPairResponse`/`TokenPair` konsistent `{ accessToken, refreshToken }` über Application/Host/Frontend. `NumberBlockAllocator.Allocate(...)`-Signatur identisch in Task 6 (Definition) und Task 14 (Aufruf).

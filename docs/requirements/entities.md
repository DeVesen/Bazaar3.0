...
| **Verkäufer** | ✅ | ✅ | Voranmelde-App: Login-Identität (`passwordHash`, Invite-Felder, `isAdmin`). Haupt-App: eigene Konditionsfelder (`salesCommission`, `feePerItem`), die beiden Geldsummen `intakeFeePaid` (eingenommen) und `payoutAmount` (ausgezahlt) sowie `settledAt` |

...
- [ ] **AC-7** — THE SYSTEM SHALL einen `GET /health` Endpoint bereitstellen, der ohne Auth `{ "status": "healthy" }` mit HTTP 200 zurückgibt. **Sobald die Datenbank angebunden ist** ([BPROJ-S04](BPROJ-S04-efcore-datenbank-setup.md)), prüft der Endpoint sie mit und antwortet bei fehlender Verbindung `503` — hier entsteht er zunächst ohne diese Prüfung, weil es noch keinen `DbContext` gibt.

...
Eigene, neue Komponente (Details → [`login-info-panel`](../../components/login-info-panel.md)) — **nicht** zu verwechseln mit dem Shared-`info-area`-Component (das ist der Feedback-Banner success/error/warn/info, etwas völlig anderes).
...
| **Markdown-Info** | Admin-konfigurierter Info-Text, gerendert über die app-lokale Custom-Component `markdown-text` (Details → [`markdown-text`](../../components/markdown-text.md); auch in Epic_Home_Verkaeufer/Epic_Home_Admin und in der Vorschau von Epic_Einstellungen genutzt) | 13 px, line-height 1.7 |
...
passiert): verbindlich in [`markdown-text`](../../components/markdown-text.md)
...
Details → [`login-form`](../../components/login-form.md). Container: `p-card`.
...
Link auf der Login-Seite → eigene Registrierungsseite (`/register`, öffentlich). Details → [`registrierung-form`](../../components/registrierung-form.md), [`password-strength-meter`](../../components/password-strength-meter.md).
...
3b. IF `defaultTypeId` in den Einstellungen nicht gesetzt ist → `409` „Registrierung ist noch nicht freigeschaltet" (ohne Typ kann das Pflichtfeld `sellerTypeId` nicht gefüllt werden).
...
| `POST /api/auth/refresh` | `public` | `{ refreshToken }` → `200` Token-Hülle (Rotation: Zeile löschen + neue anlegen) / `401` bei unbekanntem, abgelaufenem oder schon rotiertem Refresh-Token |
...
- **Blacklist für Access-Tokens.** Ein ausgegebenes Access-Token bleibt bis `exp` (5 Tage) gültig; es gibt kein `POST /api/auth/logout`, Logout löscht nur den localStorage (VSHELL-S04 AC-9).
  **Nicht** out of scope ist die Refresh-Token-Invalidierung: Refresh-Tokens liegen als Hash in einer eigenen Tabelle (eine Zeile pro Sitzung, siehe [`entities/refresh-token.md`](../../entities/refresh-token.md)), rotieren bei jedem `/refresh` und werden bei `PUT /api/profile/password` für **alle** Geräte gelöscht.
- **Geräte-Übersicht in der UI** („hier bist du angemeldet") und gezieltes Abmelden einer einzelnen Sitzung. Mehrere parallele Sitzungen sind unterstützt (max. 5 je Verkäufer), nur nicht sichtbar/verwaltbar.

...
- 4. Filter — Freitext + Original/Neu
...
Component-Details → [`master-data-popup`](../../../../components/master-data-popup/component.md) (Ausprägung Kategorie)
...
5. **AC-5** — WHEN der Nutzer im Freitext-Filter tippt, THEN SHALL das System die Tabelle 300 ms nach der letzten Eingabe auf Kategorien filtern, deren Name die Eingabe (case-insensitiv) enthält.
6. **AC-6** — WHEN der Nutzer im Original/Neu-Filter eine Option wählt, THEN SHALL das System die Tabelle sofort auf Kategorien mit passendem `original`-Wert filtern.

...
**In Scope:** Jest + `jest-preset-angular` im Frontend, xUnit v3 + FluentAssertions + Moq
im Backend, Testprojekt-Struktur, `Bazaar.Architecture.Tests` mit NetArchTest, ein
...
Testprojekte spiegeln die Produktionsstruktur 1:1. Namensschema der Testmethoden:
`<Methode>_<Situation>_<Erwartung>`, Aufbau strikt Arrange-Act-Assert.
...
- [ ] **AC-3** — THE SYSTEM SHALL `Bazaar.Api.IntegrationTests` über `WebApplicationFactory` gegen die echte Endpoint-Registrierung testen; die Datenbank SHALL dabei über einen PostgreSQL-Container (Testcontainers) bereitgestellt werden, nicht über In-Memory-Provider.
